...
public class AuthService
{
    ...
    public AuthService(ILogger<AuthService> logger,
            IConnectionMultiplexer connection, IConfiguration configuration,
    ...
    public async Task LogOut(int userId, string sessionId)
    {
        await DeleteSessionCache(sessionId, userId);
    }
    ...
    public async Task SaveSessionCache(string sessionId, int userId, string tokenFamilyUuid)
    {
        var db = RedisConnection.GetDatabase();

        var cacheKey = GetCacheKey(sessionId, userId);

        await db.StringSetAsync(cacheKey, tokenFamilyUuid);
    }

    public async Task DeleteSessionCache(string sessionId, int userId)
    {
        var db = RedisConnection.GetDatabase();
    ...
    private static RedisKey GetCacheKey(string sessionId, int userId) => new RedisKey($"Session:{sessionId}:{userId}");
